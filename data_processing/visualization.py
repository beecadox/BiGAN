...
def plot_colorxyzs(xyzs, zdir='y', cmap='YlGn', xlim=(-0.3, 0.3), ylim=(-0.3, 0.3), zlim=(-0.3, 0.3)):
    fig = plt.figure()
...
def plot_xyz(xyz, zdir='y', cmap='YlGn', xlim=(-0.3, 0.3), ylim=(-0.3, 0.3), zlim=(-0.3, 0.3)):
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')
    elev = 30
    azim = -45
...
def plot_pcds_patterns(filename, pcds, titles, use_color=[], color=None, suptitle='', sizes=None, cmap='YlGn', zdir='y',
                       xlim=(-0.3, 0.3), ylim=(-0.3, 0.3), zlim=(-0.3, 0.3)):
    if sizes is None:
        sizes = [5 for i in range(len(pcds))]
    fig = plt.figure(figsize=(len(pcds) * 3, 3))
    for i in range(1):
        ...
        for j, (pcd, size) in enumerate(zip(pcds, sizes)):
            ...
            ax.scatter(pcd[:, 0], pcd[:, 1], pcd[:, 2], zdir=zdir, color=rgba, s=clr[:, 4], cmap=cmap, vmin=-1,
                       vmax=0.5)
            ax.set_title(titles[j])
            ax.set_axis_off()
            ax.set_xlim(xlim)
